...
export default function Index({ isRule, isParamAsShow, formulas, ractions, setParamAsVisible, saveFormula }) {
  const theme = useStore(state => state.theme);
  const [query, setQuery] = useState('$');
  const [isvisible, setIsVisible] = useState(isParamAsShow);
    ...
    if (name === '') {
      ...
      if (isRule) {
        for (let j = 0; j < ractions.length; j++) {
          let item = [];
          item.push(ractions[j].id, ractions[j].action_name, ractions[j].color, ractions[j].price, ractions[j].product_name);
      ...
      let encoded;
      if (isRule) {
        encoded = NameContract.methods.pushRule(name, save_actions, save_params).encodeABI();
      }
      else {
        encoded = NameContract.methods.pushFunction(name, save_params).encodeABI();
      }
